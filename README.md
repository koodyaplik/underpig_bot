...
Команда сразу ищет рейс на текущую дату в `BOT_DEFAULT_TIMEZONE`. Выбор даты и календарь отсутствуют. Обычное сообщение `FV6106` поиск не запускает.

Запрос Aviationstack содержит только два query-параметра:

```text
GET /v1/flights?access_key=KEY&flight_iata=FV6106
```

Полный URL, включая Aviationstack API key, записывается в INFO-лог. Дата фильтруется локально из ответа API.
...
«Сегодня» вычисляется в `BOT_DEFAULT_TIMEZONE`. После получения рейса время вылета и прилета отображается в timezone соответствующего аэропорта.
...
- отслеживаются только рейсы на текущую дату;
- API-запрос содержит только `access_key` и `flight_iata`.
